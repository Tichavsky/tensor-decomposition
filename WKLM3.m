function [A,B,C,iter]=WKLM3(T,W,R,m,numit,A,B,C)
%
% Weighted Krylov-Levenberg-Marquardt method for CP decomposition of order-3
% tensors. Input:
%
...
%
% Programmed by Petr Tichavsky, January 2019
%
[Ia,Ib,Ic]=size(T); 
if nargin<8
    A=randn(Ia,R);
...
ind=reshape(1:Ia*Ib*Ic,Ia,Ib,Ic);
ind=permute(ind,[2,3,1]);
ind=ind(:);
indd=ind(ind);
Y1=reshape(T,Ia,Ib*Ic);
W1=reshape(W,Ia,Ib*Ic);
iter=zeros(1,numit);
na=sum(A.^2); nb=sum(B.^2); nc=sum(C.^2);
mm=[nb.*nc na.*nc nb.*na];
mu=max(mm);  % initial parameter mu
nu=2;
err=wchyba(Y1,W,A,B,C);      %%% computes the error of the current approximation
for it=1:numit
    chba=(Y1-A*krb(C,B)').*W1;
    gA=chba*krb(C,B);
    gC=reshape(chba,Ia*Ib,Ic)'*krb(B,A);
    gB=reshape(chba(ind),Ib,Ia*Ic)*krb(A,C);
    g=[gA(:); gB(:); gC(:)];
    ...
    W=zeros(m+1,m+1); 
for i=1:m+1
    F=W1.*(gA*krb(C,B)'+A*krb(gC,B)'+A*krb(C,gB)');
    yA=F*krb(C,B);
    yB=reshape(F(ind),Ib,Ia*Ic)*krb(A,C);
    yC=reshape(F(indd),Ic,Ia*Ib)*krb(B,A);
    zz=[yA(:); yB(:); yC(:)];
    W(1:i,i)=Y(:,1:i)'*zz;
...
function [err,Y]=wchyba(X,W,A,B,C)
%
